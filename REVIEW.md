...
- Streaming handlers no longer run serially (blocking other requests for the
  same endpoint) (Go: `go func`, Python: `asyncio.create_task` + retained task
  set).
...
headers, but services generated by any version of this plugin prior to this fix
still emit `Status`/`Description`. The streaming receivers already accept both
...
## PR #10 bot review (triaged 2026-06-14; all findings resolved)

CodeRabbit/Gemini reviewed PR #10. Eight confirmed bugs were fixed first (Go
`ToMillis` sub-ms truncation; error-code identifier collisions like
...
larger follow-ups below were initially deferred and have since been fixed in
this branch as well:

- **#13 Streaming goroutines tied to service shutdown.** The generated service
  wrapper now holds a shutdown `context.CancelFunc` + `sync.WaitGroup`; each
  stream goroutine derives its `ctx` from that shutdown context and is tracked
  on the WaitGroup (`wg.Add` before `go`, `defer wg.Done`). `Stop()` stops the
  underlying micro service, cancels the context, then drains the goroutines
  (bounded by a 5s deadline so a stuck stream can't hang `Stop()`). Plumbing is
  emitted only for services that actually stream (`$hasStreaming`); `sync` is
  imported via a dedicated header guard. (go/service.go.tmpl, go/header.go.tmpl)
- **#14 Server-streaming / bidi handler failures surfaced to the client.**
  Python and TS now emit an error close-frame on the handler-error path for
  server-streaming and bidi (`close_with_error` / `closeWithError`, which set
  `Status`/`Description` — the headers the client receivers decode as a stream
  error), so the client raises instead of hanging until its own timeout. Go
  already did this via `CloseWithError`; the client-streaming paths were already
  correct. (python/service.py.tmpl, ts/service.ts.tmpl)
- **#15 Duplicated per-service KV `put` helper removed.** `client.go.tmpl` no
  longer defines its own `put<Service>ClientKVValue`; it reuses the single
  `put<Service>KVValue` from service.go.tmpl (both render into the same
  generated file, which always emits that helper). (go/client.go.tmpl)
